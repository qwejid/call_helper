...
class ReplacementMemberShortSerializer(ExtendedModelSerializer):
    ...
    full_name = serializers.CharField(source='member.employee.user.get_full_name')
    ...
    def get_description(self, instance):
        ...
        if break_start > now:
            ...
            return f'Обед начнётся через {convert_timedelta_to_str_time(delta)}'
        elif break_end > now:
            delta = break_end - now
            return f'Обед завершится через {convert_timedelta_to_str_time(delta)}'
        return None
